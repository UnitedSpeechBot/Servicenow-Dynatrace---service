...
class PaymentProcessor:
    ...
    def __init__(self, origin_id: str = "dt0c01.AUTO_GENERATED"):
        ...
        self.smtp_hosts = ["smtp.internal:587", "smtp.backup.internal:587", "smtp.external:25"]
    ...
    def _send_email_notification(self, user_email: str, status: str):
        """Simulates sending an order confirmation email with fallback SMTP servers."""
        # Try each SMTP server in the list until one works
        for attempt in range(len(self.smtp_hosts)):
            smtp_host = self.smtp_hosts[self.current_smtp_index]
            try:
                logging.info(f"Sending {status} email to {user_email} via {smtp_host}...")
                # Simulation of connection refusal for internal servers
                if "internal" in smtp_host:
                    # For demo purposes, we'll still simulate failure but handle it gracefully
                    raise ConnectionRefusedError(f"SMTP connection refused at {smtp_host}")
                # If we reach here with external SMTP, it would succeed
                ...
                # Try the next SMTP server in the list
                self.current_smtp_index = (self.current_smtp_index + 1) % len(self.smtp_hosts)
        
        # If we've tried all SMTP servers and none worked
        logging.error(f"CRITICAL: All SMTP servers failed when sending email to {user_email}")
        return False
    ...
    def authorize_payment(self, amount: float, currency: str = "USD") -> Dict:
        ...
        # Step 2: Call Gateway with retry logic
        success = False
        for attempt in range(self.max_retries):
            if self._call_external_gateway(payload):
                success = True
                break
            logging.warning(f"Gateway attempt {attempt+1}/{self.max_retries} failed. Retrying...")
