...
class PricingEngine:
    ...
    def calculate_final_price(self, base_price: float, quantity: int, state_code: str) -> float:
        ...
        try:
            ...
            logger.warning(f"Invalid quantity type received: {type(quantity)}. Converting to integer.")
        ...
        try:
            for item in items:
                ...
                try:
                    ...
                    logger.warning(f"Invalid quantity format for {pid}: {qty}. Using default of 1.")
                    qty = 1
