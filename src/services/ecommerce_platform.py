...
class DatabaseAccessLayer:
    ...
    def update_stock(self, product_id: str, quantity_change: int) -> bool:
        ...
        # ⚠️ FIXED BUG #1: ValueError for negative stock
        # Now handles negative scenarios gracefully
        if new_stock < 0:
...
class PricingEngine:
    ...
    def calculate_final_price(self, base_price: float, quantity: int, state_code: str) -> float:
        ...
        # ⚠️ FIXED BUG #2: TypeError if a string is accidentally passed as quantity
        # Added type validation/casting here
        try:
    ...
    def process_order(self, order_req: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            for item in items:
                ...
                # Convert qty to int to avoid TypeError
                try:
                    qty_int = int(qty)
                except (ValueError, TypeError):
                ...
                # Deduct inventory count with the converted integer value
                self.db.update_stock(pid, -qty_int)
                
                # Calculate price with the converted integer value
