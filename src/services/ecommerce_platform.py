...
class PricingEngine:
    ...
    def calculate_final_price(self, base_price: float, quantity: int, state_code: str) -> float:
        ...
        try:
            ...
            logger.warning(f"Invalid quantity type provided: {quantity}. Converting to integer 1.")
            quantity = 1
        ...
        try:
            for item in items:
                ...
                try:
                    ...
                    logger.warning(f"Invalid quantity type provided for {pid}: {qty}. Converting to integer 1.")
                    qty = 1
