...
class DatabaseAccessLayer:
    ...
    def update_stock(self, product_id: str, quantity_change: int) -> bool:
        ...
        # Fixed bug: Handle negative stock gracefully
        if new_stock < 0:
...
class PricingEngine:
    ...
    def calculate_final_price(self, base_price: float, quantity: Union[int, str], state_code: str) -> float:
        """Compute the final total for an order line item."""
        
        # Fixed bug: Add type validation/casting for quantity
        try:
        ...
        try:
            for item in items:
                ...
                try:
                    ...
                    logger.warning(f"Invalid quantity for {pid}: {qty}. Using default of 1.")
                    qty = 1
